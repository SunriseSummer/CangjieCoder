...
def run_tests(client_factory):
    ...
    c.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 1
    ...
    c.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 5
    resp = c.execute()

    tests_s1 = [
        ("replace_text_basic", lambda: resp[0]["ok"] is True),
        ("replace_text_verify", lambda: (
            "func sum(" in resp[1]["data"]["content"]
        )),
        ...
        ("rollback_content_restored", lambda: (
            resp[5]["data"]["content"] == orig_utils
        )),
    ]
    ...
    c3.call_tool("workspace.replace_text", {                     # 1
        "path": "src/utils.cj",
        "oldText": "func multiply(",
        "newText": "func mul("
    })
    c3.call_tool("workspace.read_file", {"path": "src/main.cj"})  # 2
    c3.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 3
    c3.call_tool("workspace.rollback")                             # 4
    c3.call_tool("workspace.read_file", {"path": "src/main.cj"})  # 5
    c3.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 6
    resp3 = c3.execute()

    tests_s3 = [
        ("multi_edit_main", lambda: resp3[0]["ok"] is True),
        ("multi_edit_utils", lambda: resp3[1]["ok"] is True),
        ("multi_edit_verify_main", lambda: (
            'greet("Test")' in resp3[2]["data"]["content"]
        )),
        ("multi_edit_verify_utils", lambda: (
            "func mul(" in resp3[3]["data"]["content"]
        )),
        ("multi_rollback", lambda: resp3[4]["ok"] is True),
        ("multi_restored_main", lambda: (
            resp3[5]["data"]["content"] == orig_main
        )),
        ("multi_restored_utils", lambda: (
            resp3[6]["data"]["content"] == orig_utils
        )),
    ]
    ...
    c4.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 1
    c4.call_tool("workspace.rollback")                              # 2
    c4.call_tool("workspace.read_file", {"path": "src/utils.cj"})  # 3
    resp4 = c4.execute()
    ...
    tests_s4 = [
        ...
        ("overwrite_restored", lambda: (
            resp4[3]["data"]["content"] == orig_utils
        )),
    ]
