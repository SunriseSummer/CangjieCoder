...
- [项目模板](#项目模板)
  ...
  - [project.bootstrap](#26-projectbootstrap)
- [事务回滚](#事务回滚)
  - [workspace.rollback](#27-workspacerollback)
...
`service` 是 CangjieCoder 的底层工具服务，通过 **stdio MCP**（Model Context Protocol）对外暴露 27 个工具。这些工具覆盖文件读写、文本搜索替换、构建测试、AST 解析与编辑、AST 结构摘要、LSP 语义查询、技能检索与批量查询、项目模板和事务回滚等能力。
...
| 26 | `project.bootstrap` | 模板 | 按项目 id 引导任意内置示例项目到工作区 |
| 27 | `workspace.rollback` | 回滚 | 回滚所有文件修改 |
...
**实现原理**：`listExampleProjects()` 返回示例项目列表（当前包含 `json_parser`、`todo_list`、`calculator`），每个项目包含 id、名称、描述和模板路径。模板文件存放在 `tests/` 目录中。
...
**实现原理**：`bootstrapJsonParserProject()` 验证目标路径在仓库内且不已存在，然后递归复制模板目录（`tests/json_parser`）到目标路径，并清理复制过来的 `target/` 和 `cjpm.lock` 文件。
...
### 26. `project.bootstrap`

**功能**：按项目 id 将任意内置示例项目复制到工作区指定路径。使用 `project.list_examples` 获取可用项目 id 列表。

**实现原理**：`bootstrapExampleProject()` 是通用引导函数，根据 `project_id` 查找对应模板并执行与 `bootstrap_json_parser` 相同的复制、清理流程。

**参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `project_id` | string | ✅ | 示例项目 id（如 json_parser、todo_list、calculator） |
| `path` | string | ✅ | 工作区内的目标目录 |

**请求示例**：
```json
{"method":"tools/call","params":{"name":"project.bootstrap","arguments":{"project_id":"todo_list","path":"demo/my-todo"}}}
```
